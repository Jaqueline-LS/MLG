envel_norm_log<-function(fit.model,alfa=0.05)
{
  par(mfrow=c(1,1))
  ...
  r <- resid(fit.model)
  tsi <- r/(si*sqrt(1-h))
  mu=exp(as.vector(X%*%matrix(fit.model$coefficients,nrow=ncol(X),1))) # Link log
  ...
  for(jj in 1:100){
    ...
    if (inter!=0) fit <- glm(resp ~ X, family=gaussian(link=log))
    else fit <- glm(resp ~ -1+ X, family=gaussian(link=log))
    X=model.matrix(fit)
    w <- fit$weights
    W <- diag(w)
    H <- solve(t(X)%*%W%*%X)
