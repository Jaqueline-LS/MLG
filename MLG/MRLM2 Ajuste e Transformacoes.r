...
# Qualidade do ajuste
plot(res_stud,ylab="Resíduo Studentizado")
identify(res_stud,n=1)

plot(yest,res_pad,xlab="Valores Ajustados", ylab="Resíduos Padronizados")
qqnorm(res_pad,xlab="Score Normal",ylab="Resíduo Padronizado")

fit.model <- fit
source("envel_norm.txt")
# identify(tsi): não consigo buscar quem é o argumento, pois só se tem qqnorm()
