rm(list=ls(all=TRUE))
# Fazer ajustes de modelos de regressão verificar que variáveis deveriam entrar no modelo

# Exemplo: Charnet, pág. 285/286
...
par(mfrow=c(2,2))
plot(cintura,res_pad) # Verificar se ainda existe relação entre X_k e y (não explicada por beta_k*X_k)
plot(ultra1,res_pad)
plot(ultra2,res_pad)
plot(prega,res_pad)
...
# Fazer análise de diagnóstico

# Indícios de heterocedasticidade => Regressão ponderada (Charnet, pag. 293)